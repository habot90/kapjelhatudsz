...
/* eslint-disable react-hooks/purity, react-hooks/immutability -- Randomness, clocks and mutable actors live inside the imperative Leaflet simulation engine. */
...
type PracticeVehicleState = "driving" | "dismounted" | "switching";
type PracticeSwitchKind = "prearranged" | "hitchhike" | null;
type PracticeCar = { id: number; point: LatLng; marker: Marker };
type Runner = {
  ...
  vehicleState: PracticeVehicleState; switchKind: PracticeSwitchKind; switchTotal: number; vehicleCycle: number; selectedCarId: number | null;
...
function PracticeGame({onBack,cityId,playerRole}:{onBack:()=>void;cityId:string;playerRole:PracticeRole}){
  ...
  const [lastCivilian,setLastCivilian]=useState<{point:LatLng;accuracy:"confirmed"|"uncertain"|"misleading"}|null>(null);
  const [placingCar,setPlacingCar]=useState(false),[selectedCarId,setSelectedCarId]=useState<number|null>(null);
  ...
  const civilianReport=()=>{
    ...
    const roll=Math.random(),accuracy=roll<.48?"confirmed":roll<.78?"uncertain":"misleading";
    const target=playerRole==="hunter"?alive[Math.floor(Math.random()*alive.length)]:null;
    const actual=target?.pos??engine.hunter.pos;
    const center=accuracy==="confirmed"?offsetPoint(actual,randomBetween(35,125)):accuracy==="uncertain"?offsetPoint(actual,randomBetween(180,600)):offsetPoint(ZONES[engine.phase].center,ZONES[engine.phase].radius*.85);
    const uncertainty=accuracy==="confirmed"?170:accuracy==="uncertain"?520:900;
    const layer=L.circle(center,{radius:uncertainty,color:accuracy==="misleading"?"#b45cff":"#f39a52",weight:2,dashArray:"5 8",fillColor:accuracy==="misleading"?"#b45cff":"#f39a52",fillOpacity:.08}).addTo(map).bindTooltip(`Civil jelentés · ${accuracy==="confirmed"?"határozott":accuracy==="uncertain"?"bizonytalan":"gyanús"}`);engine.layers.push({layer,expires:engine.simTime+90});setLastCivilian({point:center,accuracy});
    const text=playerRole==="hunter"?`Civil hívás: egy ${target?.vehicle??"autó"} ${target?directionOf(target):"ismeretlen irányba"} tartott.`:"Civil hívás: az üldöző autóját látták ezen a környéken.";addFeed(text);setCallAlert(text);window.setTimeout(()=>setCallAlert(null),3500);
  };
  ...
  useEffect(()=>{
    ...
    void import("leaflet").then(L=>{
      ...
      const hunterIcon=L.divIcon({className:"token-wrap hunter-wrap",html:`<span class="hunter-token">◎</span><b>${playerRole==="hunter"?"TE · ÜLDÖZŐ":"GÉPI ÜLDÖZŐ"}</b>`,iconSize:[76,58],iconAnchor:[38,27]});hunterMarker.current=L.marker(HUNTER_START,{icon:hunterIcon,opacity:playerRole==="hunter"?1:0,zIndexOffset:1100}).addTo(map);
      ...
      map.on("click",async e=>{const engine=engineRef.current;if(!engine||engine.paused||engine.finished)return;const mover=playerRole==="hunter"?engine.hunter:engine.runners[0];if(!mover||mover.caught)return;const target:[number,number]=[e.latlng.lat,e.latlng.lng];if(map.distance(mover.pos,target)>12000){setRouteMessage("Egyszerre legfeljebb 12 km-es útszakaszt válassz");return}const requestId=++mover.requestId,generation=engine.generation;
        if(playerRole==="runner"&&placingCarRef.current){
          ...
          if(!pointInZone(target,engine.phase)){setRouteMessage("Egyeztetett autót csak az aktív zónában rakhatsz le.");return}
          setRouteMessage("Az autó közúti helyének ellenőrzése…");
          try{const route=await requestRoute(mover.pos,target,generation);if(mover.requestId!==requestId)return;const point=route.coords[route.coords.length-1]??target;const id=Date.now()+parkedCars.current.length;const icon=L.divIcon({className:"practice-parked-car",html:`<span>🚙</span><b>${parkedCars.current.length+1}</b>`,iconSize:[38,38],iconAnchor:[19,19]});const marker=L.marker(point,{icon,zIndexOffset:650}).addTo(map).bindTooltip(`Egyeztetett autó ${parkedCars.current.length+1}`);marker.on("click",event=>{L.DomEvent.stopPropagation(event);setSelectedCarId(id);setRouteMessage("Egyeztetett autó kiválasztva.")});parkedCars.current.push({id,point,marker});setSelectedCarId(id);setCarViews(parkedCars.current.map(car=>({id:car.id,point:car.point,distance:Math.round(map.distance(mover.pos,car.point))})));placingCarRef.current=false;setPlacingCar(false);setRouteMessage(`Autó elhelyezve · ${parkedCars.current.length}/10`)}catch{setRouteMessage("Erre a pontra nem sikerült közúti autót elhelyezni")}return
        }
        if(playerRole==="runner"&&engine.runners[0].vehicleState==="switching"){setRouteMessage("Autóváltás közben nem mozoghatsz.");return}
      ...
      timer=window.setInterval(()=>{
        ...
        for(const r of alive){
          if(playerRole==="runner"&&r.id===0){
            if(r.vehicleState==="switching"){r.switching-=dt;if(r.switching<=0){if(r.switchKind==="prearranged"&&r.selectedCarId!==null){const used=parkedCars.current.find(car=>car.id===r.selectedCarId);used?.marker.remove();parkedCars.current=parkedCars.current.filter(car=>car.id!==r.selectedCarId);setSelectedCarId(null)}r.selectedCarId=null;r.vehicleState="driving";r.switchKind=null;r.switchTotal=0;r.switching=0;r.carLeft=CAR_SECONDS;r.vehicleCycle++;r.vehicle=VEHICLES[Math.floor(Math.random()*VEHICLES.length)];addFeed("Új autóban vagy. Elindult az új ötperces ciklus.")}continue}
            const runnerFactor=locked?.id===r.id?(.72+.28*closeRatio):1;
            if(r.vehicleState==="driving")r.carLeft-=dt;const done=advance(r,(r.vehicleState==="dismounted"?WALKING_SPEED:r.speed)*runnerFactor*dt,map);r.marker.setLatLng(r.pos);if(done&&r.route.length){r.route=[];r.routeIndex=0;hunterLine.current?.remove();hunterLine.current=null;setRouteMessage("Válassz új menekülési irányt a térképen")}continue
          }
          ...
          const factor=locked?.id===r.id?(.72+.28*closeRatio):1,done=advance(r,r.speed*factor*dt,map);r.marker.setLatLng(r.pos);
  ...
  const toggleCarPlacement=()=>{const playerRunner=engineRef.current?.runners[0];if(!playerRunner||playerRunner.vehicleState!=="driving")return;const next=!placingCarRef.current;placingCarRef.current=next;setPlacingCar(next);setRouteMessage(next?"Koppints az aktív zónán belül egy útra az autó lerakásához.":"Autólerakás kikapcsolva.")};
  const exitPracticeVehicle=()=>{const playerRunner=engineRef.current?.runners[0];if(!playerRunner||playerRunner.vehicleState!=="driving")return;playerRunner.vehicleState="dismounted";playerRunner.route=[];playerRunner.routeIndex=0;hunterLine.current?.remove();hunterLine.current=null;setVehicleView(value=>({...value,vehicleState:"dismounted"}));addFeed("Kiszálltál. Gyalog továbbmehetsz, egyeztetett autóba ülhetsz vagy stoppolhatsz.")};
  const startPracticeSwitch=(kind:Exclude<PracticeSwitchKind,null>)=>{const playerRunner=engineRef.current?.runners[0];if(!playerRunner||playerRunner.vehicleState!=="dismounted")return;if(kind==="prearranged"&&(!selectedCar||selectedCarDistance===null||selectedCarDistance>75)){setCallAlert("Válassz egy legfeljebb 75 méterre lévő egyeztetett autót.");window.setTimeout(()=>setCallAlert(null),2800);return}playerRunner.vehicleState="switching";playerRunner.switchKind=kind;playerRunner.selectedCarId=kind==="prearranged"?selectedCarId:null;playerRunner.switching=kind==="prearranged"?10:randomBetween(25,45);playerRunner.switchTotal=playerRunner.switching;setVehicleView({vehicleState:"switching",switchKind:kind,carLeft:playerRunner.carLeft,switching:playerRunner.switching,vehicleCycle:playerRunner.vehicleCycle});addFeed(kind==="prearranged"?"Megkezdted az egyeztetett autó átvételét.":"Stoppolsz. Maradj egy helyben, amíg érkezik autó.")};
  const removePracticeCar=(id:number)=>{const car=parkedCars.current.find(item=>item.id===id);car?.marker.remove();parkedCars.current=parkedCars.current.filter(item=>item.id!==id);if(selectedCarId===id)setSelectedCarId(null);setCarViews(views=>views.filter(item=>item.id!==id))};
  ...
  return <main className={`page chase-${closeLevel}`}><section className="game">
    ...
    <div className="game-layout">
      ...
      <aside className="sidebar">
        {playerRole==="runner"&&<section className={`practice-vehicle panel ${vehicleView.vehicleState} ${vehicleView.carLeft<=0?"overdue":""}`}><div className="panel-heading"><span className="eyebrow">AUTÓ · {String(vehicleView.vehicleCycle+1).padStart(2,"0")}</span><b className="vehicle-live-state"><i/>{vehicleView.carLeft<=0&&vehicleView.vehicleState==="driving"?"ÉLŐBEN LÁTHATÓ":vehicleView.vehicleState==="driving"?"MENETBEN":vehicleView.vehicleState==="dismounted"?"GYALOG":"MOZDULATLAN"}</b></div><div className="practice-vehicle-clock">{vehicleView.vehicleState==="switching"?formatTime(vehicleView.switching):vehicleView.vehicleState==="dismounted"?"5 KM/H":vehicleView.carLeft<=0?"TÚLIDŐ":formatTime(vehicleView.carLeft)}</div><p>{vehicleView.vehicleState==="driving"?(vehicleView.carLeft<=0?"Az üldöző élőben lát, amíg ki nem szállsz.":"Öt perc után autót kell cserélned."):vehicleView.vehicleState==="dismounted"?"Gyalog haladsz az utakon; közelíts meg egy autót vagy stoppolj.":vehicleView.switchKind==="prearranged"?"Autóátvétel folyamatban.":"Stoppolt autóra vársz."}</p><div className="practice-car-actions"><button type="button" className={placingCar?"active":""} disabled={vehicleView.vehicleState!=="driving"||carViews.length>=10} onClick={toggleCarPlacement}>🚙 AUTÓ LERAKÁSA <small>{carViews.length}/10</small></button>{vehicleView.vehicleState==="driving"?<button type="button" className="danger" onClick={exitPracticeVehicle}>KISZÁLLOK</button>:vehicleView.vehicleState==="dismounted"?<><button type="button" disabled={!selectedCar||selectedCarDistance===null||selectedCarDistance>75} onClick={()=>startPracticeSwitch("prearranged")}>EGYEZTETETT AUTÓBA ÜLÖK <small>{selectedCarDistance===null?"VÁLASSZ AUTÓT":`${selectedCarDistance} M`}</small></button><button type="button" onClick={()=>startPracticeSwitch("hitchhike")}>STOPPOLOK <small>25–45 MP</small></button></>:null}</div>{carViews.length>0&&<div className="practice-car-list">{carViews.map((car,index)=><div key={car.id} className={selectedCarId===car.id?"selected":""}><button type="button" onClick={()=>{setSelectedCarId(car.id);mapRef.current?.setView(car.point,16,{animate:true})}}><b>AUTÓ {index+1}</b><small>{car.distance} m</small></button><button type="button" aria-label={`Autó ${index+1} törlése`} onClick={()=>removePracticeCar(car.id)}>×</button></div>)}</div>}</section>}
        <section className={`strategy-panel panel ${playerRole}`}><div className="panel-heading"><span className="eyebrow">{playerRole==="hunter"?"ÜLDÖZŐI PARANCSKÖZPONT":"MENEKÜLÉSI TERV"}</span><b className="sensor-state"><i/>TÁVOLSÁG REJTVE</b></div><div className="practice-intel"><span>{lastCivilian?"☎":"…"}</span><p><strong>{lastCivilian?`CIVIL JELENTÉS · ${lastCivilian.accuracy==="confirmed"?"HATÁROZOTT":lastCivilian.accuracy==="uncertain"?"BIZONYTALAN":"GYANÚS"}`:"NINCS AKTÍV CIVIL JELENTÉS"}</strong><small>A hívás helye lehet pontos, bizonytalan vagy szándékosan félrevezető.</small></p></div><div className="practice-strategy-buttons"><button type="button" onClick={focusCivilian}><span>☎</span><b>{playerRole==="hunter"?"CIVIL NYOM":"VESZÉLYJEL"}</b><small>TÉRKÉPEN</small></button><button type="button" onClick={focusOfficial}><span>⌁</span><b>HIVATALOS JEL</b><small>10 PERCES</small></button><button type="button" onClick={focusZone}><span>▱</span><b>{playerRole==="hunter"?"KERESÉSI ZÓNA":"MENEKÜLÉSI ZÓNA"}</b><small>AKTÍV</small></button></div><p className="hidden-distance-rule">Az ellenfél távolsága és iránya rejtett. 50 méteren belül az elfogás automatikus.</p></section>
        ...
        <section className="rules panel"><div><span className="rule-code">05</span><p><strong>AUTÓLIMIT</strong><small>5:00 után csere vagy stoppolás</small></p><i className="rule-status">KÖTELEZŐ</i></div><div><span className="rule-code">10</span><p><strong>HELYZETJEL</strong><small>Pontos, de csak pillanatfelvétel</small></p><i className="rule-status">PERCENKÉNT</i></div></section>
...
function PracticeSetup({onBack,onStart}:{onBack:()=>void;onStart:(config:PracticeConfig)=>void}){
  ...
  return <main className="page practice-setup-page"><section className="practice-setup panel">
    ...
    <fieldset><legend>3. SZEREP</legend><div className="practice-choice-grid two"><button type="button" className={role==="hunter"?"selected hunter-choice":"hunter-choice"} aria-pressed={role==="hunter"} onClick={()=>setRole("hunter")}><strong>◎ ÜLDÖZŐ VAGYOK</strong><small>Keresési jelekből kutatod fel a gépi menekülőket.</small></button><button type="button" className={role==="runner"?"selected runner-choice":"runner-choice"} aria-pressed={role==="runner"} onClick={()=>setRole("runner")}><strong>◆ MENEKÜLŐ VAGYOK</strong><small>Civil veszélyjelzések alapján menekülsz a gépi üldöző elől.</small></button></div></fieldset>
