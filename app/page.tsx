...
function PracticeGame({onBack,cityId,playerRole}:{onBack:()=>void;cityId:string;playerRole:PracticeRole}){
  ...
  const [hud,setHud]=useState({gameLeft:GAME_SECONDS,zoneLeft:ZONE_SECONDS,signalLeft:SIGNAL_SECONDS,civilianLeft:45,phase:0,captured:0,alive:playerRole==="hunter"?10:1,current:ZONES[0].name,next:ZONES[1].name});
  ...
  const fireSignal=()=>{
    ...
    setCallAlert("HIVATALOS HELYZETJELENTÉS");window.setTimeout(()=>setCallAlert(null),2200);
  };
  ...
  useEffect(()=>{
    ...
    void import("leaflet").then(L=>{
      ...
      timer=window.setInterval(()=>{
        ...
        if(engine.simTime-engine.lastHudAt>=1){engine.lastNearestId=nearest?.id??null;engine.lastNearestDistance=nearestDistance;engine.lastHudAt=engine.simTime;setHud({gameLeft:engine.gameLeft,zoneLeft:engine.zoneLeft,signalLeft:engine.signalLeft,civilianLeft:engine.civilianLeft,phase:engine.phase,captured:engine.captured,alive:alive.length,current:ZONES[engine.phase].name,next:ZONES[Math.min(engine.phase+1,ZONES.length-1)].name})}
      },100);
    });
  ...
  return <main className={`page chase-${closeLevel}`}><section className="game">
    ...
    <div className="game-layout">
      <div className="map-shell"><div ref={mapNode} className="real-map" aria-label={`${city.name} valós térképes gyakorló üldözése`}/><div className="map-grid" aria-hidden="true"/><div className="map-corners" aria-hidden="true"><i/><i/><i/><i/></div>
        <div className="map-hud signal"><div className="signal-orbit"><span className="pulse"/></div><div className="signal-copy"><span className="hud-kicker">SAT // PING</span><strong>HELYZETJEL <b>{formatTime(hud.signalLeft)}</b></strong><small>{playerRole==="hunter"?"A menekülők":"Az üldöző"} 10 perces pillanatképéig</small><div className="signal-meter"><i style={{width:`${Math.max(0,Math.min(100,(1-hud.signalLeft/SIGNAL_SECONDS)*100))}%`}}/></div></div></div>
        <div className="map-hud civilian-countdown"><span>☎</span><div><small>KÖVETKEZŐ CIVIL HÍVÁS</small><strong>{formatTime(hud.civilianLeft)}</strong></div></div>
